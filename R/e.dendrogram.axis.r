...
e.dendrogram.axis<-function(main,ylab,yaxes,ylim,xaxes,xdper,dmai,peaks,x){
	...
	if(!length(peaks)==1)
		plot(1,1,type="n",main=main,xlab="",ylab=ylab,xlim=c(xleft-0.3,xright+0.2),ylim=ylim,axes=F)

	else
		plot(1,1,type="n",main=main,xlab="",ylab=ylab,xlim=c(xleft-0.3,xright),ylim=ylim,axes=F)
	...
	if(xaxes){
		...
		axis(side=1,at=c(xleft-0.3,median(c(xleft-0.3,xright)),xright),labels=measure,lty=3)
	}
}
