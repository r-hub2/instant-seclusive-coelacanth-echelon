...
echepoi<-function(echelon.obj,cas,pop=NULL,ex=NULL,K=length(cas)/2,Kmin=1,n.sim=99,
...
#############################
	...
	log.lambda <- cz*log(cz/ez) + (cg-cz)*log((cg-cz)/(eg-ez))
...
##############################
	...
	if(n.sim > 0){
		...
		sim_lambda <- e_monteCPP(x=nulldata,rin=rin, K=K, Kmin = floor(Kmin), par1=ex, par2=pop2, type=type)
